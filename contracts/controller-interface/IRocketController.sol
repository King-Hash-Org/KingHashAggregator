...
interface IRocketController   {
    /**
    *@dev Adds `rEthBalance` shares to `rEthBalanceMap` that keep tracks of RETH Tokens owned by each address.
    *@param userAddress : the user's unique address
    * Requirements 
    * - `userAddress` cannot be the zero address.
    * - `msg.sender` must be inside list of allowed addresses. 
    */
    function addREthBalance(address userAddress, uint256 rEthBalance) external  ;

    /**
     * @dev Returns the balance of RETH Tokens owned by the address `userAddress`.
     * @param userAddress : the user's unique address
     * Requirements:
