    ...
    OwnableUpgradeable
{
     ...
     */
    function stake(bytes[] calldata data) external payable override nonReentrant whenNotPaused returns (bool) {
    ...
    function liquidStake(
        ...
        address operator
    ) external payable override nonReentrant onlyKingHashLiqStaking whenNotPaused {
        super.liquidRoute(pubkey, withdrawalCredentials, signature, depositDataRoot, operator);
    }

    /**
     * @dev See {IAggregator-unstake}.
     */
    function unstake(bytes[] calldata data) external payable override nonReentrant whenNotPaused returns (bool) {
